...
export default function Sidebar({ isExpanded, setIsExpanded, posts, onIncidentClick }: SidebarProps) {
  ...
  return (
    <div
      className={`p-4 flex flex-col gap-4 transition-all duration-300 ease-in-out ${
        isExpanded ? "w-72" : "w-20"
      }`}
    >
      {/* Logo Section */}
      <div className="bg-white/95 backdrop-blur-md rounded-2xl shadow-lg border border-gray-200/20 p-4">
      ...
      {/* Search and Incidents Section */}
      {isExpanded && (
        <div className="bg-white/95 backdrop-blur-md rounded-2xl shadow-lg border border-gray-200/20 p-4 flex-1">
          ...
          <div className="space-y-4">
            ...
            <div className="space-y-2 max-h-[calc(100vh-400px)] overflow-y-auto pr-2 scrollbar-thin scrollbar-thumb-gray-300 scrollbar-track-transparent">
              {posts.map((post) => (
                <div 
                  key={post.id} 
